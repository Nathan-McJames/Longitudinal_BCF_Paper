...
List fast_lbcf2(NumericMatrix X_mu,
               ...
               NumericVector Z_growth2,
               NumericVector Z_treat2,
               NumericMatrix X_growth2,
               NumericMatrix X_treat2,
               NumericMatrix X_mu_test,
               NumericMatrix X_growth_test,
               NumericMatrix X_treat_test,
               NumericMatrix X_growth_test2,
               NumericMatrix X_treat_test2,
               double alpha_mu,
               double beta_mu,
               double alpha_growth,
               double beta_growth,
               ...
               int min_nodesize)
{
  ...
  int p_growth2 = X_growth2.ncol();
  int p_treat2 = X_treat2.ncol();
  ...
  NumericMatrix tree_preds_growth2(n, n_tree_growth);
  NumericMatrix tree_preds_treat2(n, n_tree_treat);
  ...
  NumericMatrix tree_preds_growth_test2(n_test, n_tree_growth);
  NumericMatrix tree_preds_treat_test2(n_test, n_tree_treat);
  ...
  NumericMatrix preds_mat_growth2(n, n_iter);
  NumericMatrix preds_mat_treat2(n, n_iter);
  ...
  NumericMatrix preds_mat_growth_test2(n_test, n_iter);
  NumericMatrix preds_mat_treat_test2(n_test, n_iter);
  ...
  Forest growth_forest2(n_tree_growth, 1, n, n_test);
  
  Forest treat_forest2(n_tree_treat, 1, n, n_test);
  ...
  for(int iter = 0; iter < n_iter; iter++)
  {
    for(int tree_num = 0; tree_num < n_tree_mu; tree_num++)
    {
      ...
      -Z_growth*rowSumsWithoutColumn(tree_preds_growth, -1)
      -Z_treat*rowSumsWithoutColumn(tree_preds_treat, -1)
      -Z_growth2*rowSumsWithoutColumn(tree_preds_growth2, -1)
      -Z_treat2*rowSumsWithoutColumn(tree_preds_treat2, -1);
    ...
    for(int tree_num = 0; tree_num < n_tree_growth; tree_num++)
    {
      ...
      -Z_growth*rowSumsWithoutColumn(tree_preds_growth, tree_num)
      -Z_treat*rowSumsWithoutColumn(tree_preds_treat, -1)
      -Z_growth2*rowSumsWithoutColumn(tree_preds_growth2, -1)
      -Z_treat2*rowSumsWithoutColumn(tree_preds_treat2, -1);
    ...
    for(int tree_num = 0; tree_num < n_tree_treat; tree_num++)
    {
      NumericVector y_resid = y_scaled-rowSumsWithoutColumn(tree_preds_mu, -1)
      -Z_growth*rowSumsWithoutColumn(tree_preds_growth, -1)
      -Z_treat*rowSumsWithoutColumn(tree_preds_treat, tree_num)
      -Z_growth2*rowSumsWithoutColumn(tree_preds_growth2, -1)
      -Z_treat2*rowSumsWithoutColumn(tree_preds_treat2, -1);
    ...
    for(int tree_num = 0; tree_num < n_tree_growth; tree_num++)
    {
      ...
      -Z_growth*rowSumsWithoutColumn(tree_preds_growth, -1)
      -Z_treat*rowSumsWithoutColumn(tree_preds_treat, -1)
      -Z_growth2*rowSumsWithoutColumn(tree_preds_growth2, tree_num)
      -Z_treat2*rowSumsWithoutColumn(tree_preds_treat2, -1);
      
      String choice = sample(choices, 1)[0];
      
      Tree proposal_tree = Tree(growth_forest2.tree_vector[tree_num]);
      
      if(choice == "Grow")
      {
        proposal_tree.grow(X_growth2, X_growth_test2, p_growth2, min_nodesize);
      }
      ...
      if(choice == "Change")
      {
        proposal_tree.change(X_growth2, p_growth2);
        proposal_tree.change_update(X_growth2, X_growth_test2);
      }
      ...
      if(!proposal_tree.has_empty_nodes(min_nodesize))
      {
        ...
        double lold = growth_forest2.tree_vector[tree_num].log_lik_tau(tau_growth, 
                                                                      tau,
                                                                      alpha_growth,
                                                                      beta_growth,
                                                                      y_resid,
        ...
        if(a > R::runif(0, 1))
        {
          growth_forest2.tree_vector[tree_num] = Tree(proposal_tree);
        }
      }
      
      growth_forest2.tree_vector[tree_num].update_nodes_tau(tau, tau_growth, y_resid, Z_growth2);
      
      NumericVector tree_preds_from_iter_growth2 = growth_forest2.tree_vector[tree_num].get_predictions();
      NumericVector tree_preds_from_iter_growth_test2 = growth_forest2.tree_vector[tree_num].get_predictions_test();
      
      for(int i=0; i<n; i++)
      {
        tree_preds_growth2(i, tree_num) = tree_preds_from_iter_growth2[i];
      }
      for(int i=0; i<n_test; i++)
      {
        tree_preds_growth_test2(i, tree_num) = tree_preds_from_iter_growth_test2[i];
      }
    ...
    for(int tree_num = 0; tree_num < n_tree_treat; tree_num++)
    {
      ...
      -Z_treat*rowSumsWithoutColumn(tree_preds_treat, -1)
      -Z_growth2*rowSumsWithoutColumn(tree_preds_growth2, -1)
      -Z_treat2*rowSumsWithoutColumn(tree_preds_treat2, tree_num);
      
      String choice = sample(choices, 1)[0];
      
      Tree proposal_tree = Tree(treat_forest2.tree_vector[tree_num]);
      
      if(choice == "Grow")
      {
        proposal_tree.grow(X_treat2, X_treat_test2, p_treat2, min_nodesize);
      }
      ...
      if(choice == "Change")
      {
        proposal_tree.change(X_treat2, p_treat2);
        proposal_tree.change_update(X_treat2, X_treat_test2);
      }
      ...
      if(!proposal_tree.has_empty_nodes(min_nodesize))
      {
        ...
        double lold = treat_forest2.tree_vector[tree_num].log_lik_tau(tau_treat, 
                                                                     tau,
                                                                     alpha_treat,
                                                                     beta_treat,
                                                                     y_resid,
        ...
        if(a > R::runif(0, 1))
        {
          treat_forest2.tree_vector[tree_num] = Tree(proposal_tree);
        }
      }
      
      treat_forest2.tree_vector[tree_num].update_nodes_tau(tau, tau_treat, y_resid, Z_treat2);
      
      NumericVector tree_preds_from_iter_treat2 = treat_forest2.tree_vector[tree_num].get_predictions();
      NumericVector tree_preds_from_iter_treat_test2 = treat_forest2.tree_vector[tree_num].get_predictions_test();
      
      for(int i=0; i<n; i++)
      {
        tree_preds_treat2(i, tree_num) = tree_preds_from_iter_treat2[i];
      }
      for(int i=0; i<n_test; i++)
      {
        tree_preds_treat_test2(i, tree_num) = tree_preds_from_iter_treat_test2[i];
      }
    ...
    NumericVector iter_preds_growth2 = rowSumsWithoutColumn(tree_preds_growth2, -1);
    NumericVector iter_preds_treat2 = rowSumsWithoutColumn(tree_preds_treat2, -1);
    ...
    NumericVector iter_preds_growth_test2 = rowSumsWithoutColumn(tree_preds_growth_test2, -1);
    NumericVector iter_preds_treat_test2 = rowSumsWithoutColumn(tree_preds_treat_test2, -1);
    ...
    for(int i=0; i<n; i++)
    {
      ...
      preds_mat_growth2(i, iter) = iter_preds_growth2[i];
      preds_mat_treat2(i, iter) = iter_preds_treat2[i];
    }
    for(int i=0; i<n_test; i++)
    {
      ...
      preds_mat_growth_test2(i, iter) = iter_preds_growth_test2[i];
      preds_mat_treat_test2(i, iter) = iter_preds_treat_test2[i];
    }
    
    tau=sample_tau(n, nu, y_scaled, iter_preds_mu+Z_growth*iter_preds_growth+Z_treat*iter_preds_treat
                     +Z_growth2*iter_preds_growth2+Z_treat2*iter_preds_treat2, lambda);
  ...
  return List::create(
    ...
    Named("predictions_growth2") = preds_mat_growth2*y_sd,
    Named("predictions_treat2") = preds_mat_treat2*y_sd,
    ...
    Named("predictions_growth_test2") = preds_mat_growth_test2*y_sd,
    Named("predictions_treat_test2") = preds_mat_treat_test2*y_sd,
